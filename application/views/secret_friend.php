<div class="[ contenido ] [ container ]">
    ...
    <div class="[ row ]">
        <div class="[ col-xs-12 col-sm-8 col-md-6 ] [ center-block margin-bottom ]">
            ...
            <?php } else { ?>
                <div class="[ text-center ] [ margin-bottom ]">
                    <a href="<?php echo base_url().'secret_friends/view_video/'.$secret_friend['group_friend_id'] ?>" disabled class="btn btn-primary btn-go"><span>Dedicatoria enviada</span></a>
                </div>
            <?php } ?>

            <?php if($has_perfect_fit) { ?>
                <div class="[ text-center ] [ margin-bottom ]">
                    <a href="<?php echo base_url().'secret_friends/view_perfect_fit/'.$secret_friend['group_friend_id'] ?>" class="btn btn-primary btn-go" onclick="ga('send', 'event', 'amigosSecretos', 'click', 'verPerfectFit');"><span>Ver perfect fit</span></a>
                </div>
            <?php } ?>
             <div class="[ text-center ] [ margin-bottom ]">
                <a href="<?php echo base_url().'secret_friends/create_message/'.$secret_friend['group_friend_id']  ?>" class="btn btn-primary btn-go" onclick="ga('send', 'event', 'amigosSecretos', 'click', 'enviarMensaje');"><span>Enviar mensaje</span></a>
            </div>
        </div>
    </div>
</div><!-- login -->